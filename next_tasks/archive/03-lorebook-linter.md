...
- **Touch floors**: 44px minimum everywhere (the global ≤767px rule already raises buttons to 48px on phones); *(post-acceptance fix, 2026-09-19, user-reported)* the mute chips are standard `mat-chip-option`s (no explicit floor — the global ≤767px `.mat-mdc-standard-chip` rule raises them to 48px exactly like the entry editor's chips) and the multi-entry jumps are real `mat-mdc-button-base` buttons with the 44px desktop floor, so no bespoke floor rule remains.
...
- **Actions**: single-entry rows → trailing text button `Go to entry` (aria `Go to entry: {title}`) → `workspace.openEntry(entryId)` + close the live ref. Multi-entry rows (duplicates, cycles) → **one compact stroked button per entry** *(post-acceptance fix, 2026-09-19, user-reported — chips did not read as clickable)*: `matButton="outlined"` + class `jump-button`, entry title + trailing `north_east` Material Symbol, `aria-label="Go to entry {title}"`, clicking jumps + closes — unmistakably interactive, deliberately distinct from the decorative chips. Zero-entry rows (the perf-guard note) → icon + message only, no affordance to click into nothing.
...
3. **UI additions** *(chip mechanics amended — post-acceptance fix, 2026-09-19, user-reported)*: a chip row in the dialog for rule mute, using **the entry editor's filter-chip pattern** (`mat-chip-listbox multiple` + `mat-chip-option`, as in `entry-activation`) so both surfaces look identical: **selected (filled + check) = the check runs; deselected (outlined) = muted** — `[selected]="!chip.muted"`, `(selectionChange)` maps `$event` to mute/unmute through `LinterState.setRuleMuted` (the `setChipFlag` `isUserInput` guard). One chip per rule id **present in the unfiltered pass** (muted rules stay visible as deselected chips so they can be re-enabled), labels humanized: Invalid regex · Duplicate keys · Ignored secondary keys · Selective without secondary · Never activatable · Recursion cycles · Self-triggers · Malformed wrappers. The row renders whenever that pass has rules — **even when every rule is muted and the filtered diagnostics are empty** — with the empty state rendering below it, so an all-muted book keeps its recovery path. Every row gains a `Not an issue` affordance (icon button `do_not_disturb_on`, tooltip `Not an issue`) that adds the row's signature to `ignoredSignatures`. When `ignoredSignatures` is non-empty, a footer line renders: `N issues marked not-an-issue` + text button `Undo all` (per-issue undo is out of scope this task).
