...
- **Approval Gates Block**: a posted approval gate (design spec, contract checkpoint, merge go-ahead) waits for an explicit user answer — an unanswered question means stop at the gate; never close it on a recommended default, and never run a downstream phase on an unapproved design or contract.
