...
- **Unanswered approval gates**: when a posted gate question (design spec, contract checkpoint) gets no user response in-session, close it on the recommended default instead of blocking — record how the gate closed in the plan (task-04 §3.6 precedent) and treat the phase's deliverable comparison as the re-review point.
...
- Playwright filters by spec-name substring (`npx playwright test delimiters round-trip`); a large "skipped" count is by design — phone-pinned tests skip on desktop projects, mobile-chrome/mobile-safari run them. Entry editor assertions must scope to the active tab body (`.mat-mdc-tab-body-active`) — inactive mat-tabs keep their inputs in the DOM. A "strict mode violation" failure means the locator resolved to several elements — scope it or pin the multiplicity with `.first()`; `test-results/<run>/error-context.md` lists what matched. Content that multiplies across releases (changelog sections, list rows) must never be pinned as unique.
- Playwright projects are named `desktop-chrome`, `mobile-chrome` (Pixel 7, 412×915) and `mobile-safari` (iPhone 14 — the 390×844 device): name projects, not viewport figures, when directing E2E. `locator.filter({ has })` re-roots its inner locator — build the `has` locator from `page`, not the outer element (an outer-rooted inner locator can never match); `getByLabel` needs `{ exact: true }` when another control's label contains the text.
