...
- **Pipeline Order**: implementation agents first (`core-engine` → `ui-specialist`), then `ts-reviewer` (typing/lint review) **before** `qa-auditor`. QA runs on two tiers — this is a small app and the full E2E matrix is expensive, so E2E is an end-of-branch gate, never a per-phase tax:
  - **Per task (fast gate)**: `qa-auditor` runs build, unit tests + coverage, lint, `typecheck:e2e`, and a `desktop-chrome`-only Playwright smoke of just the specs the task touched or authored. E2E specs are still written and migrated in-task (the pinned-behavior grep rule below is unchanged) — only the full-matrix execution is deferred.
  - **Branch-final sweep (once per task branch, at its close)**: the full three-project Playwright matrix, run per project (`--project=desktop-chrome` / `--project=mobile-chrome` / `--project=mobile-safari`), plus a final `npm test --coverage` and `npm run lint`, before pushing the branch for user testing. A red spec loops back to the task that authored it (fix-forward); inside the loop re-run only the failing project/spec, and repeat the whole sweep at the end.
...
- **Verification First**: Always run the fast gates (build, unit tests + coverage, lint) before completing a phase; the full E2E matrix is the branch-close gate (see Pipeline Order).
...
- E2E & fidelity: `npx playwright test` (branch-final sweep — run per project, `--project=<name>`)
...
- Shared spec fixtures: unit specs reuse `src/testing/` (`projectOf`, `severityFixture`, `installMatchMediaStub`, and the PNG chunk builders in `png-fixtures.ts` — shared by unit and e2e card specs) and e2e specs reuse `e2e/helpers.ts` (`importLorebook`, `exportWorldInfo`, `selectFirstTwoRows`, `openExportMenu`, `exportCard`, `expectSnackbar`) instead of copy-pasting helpers into each spec (root `TEST-REPORT.md` traces the duplication this removed).
...
- CDK overlays render through the native Popover top layer (`popover="manual"`): no body-level element can stack above an open menu at any z-index — anchor injected tooltips/helper nodes inside the open overlay pane. Related trap: Material tooltips never fire on truly `disabled` buttons — the disabled-with-explanation pattern is an enabled-but-muted row (`.card-export-unavailable` opacity recipe in `styles.scss`) with `matTooltip` + `aria-description` and a click-guard that snacks the same copy (task 15 card-export rows).
